...
from models_text.mutual_info import sample_batch, mutual_information
import warnings
...
class LabelSmoothing(nn.Module):         #实现标签平滑的类
...
class NoamOpt:                #学习率调度和权重衰减的优化器包装类
...
def subsequent_mask(size):                       #生成注意力掩码
...
def create_masks(src, trg, padding_idx):              #创建输入序列掩码
...
def loss_function(x, trg, padding_idx, criterion):         #损失函数
...
def SNR_to_noise(snr):              #将信噪比（SNR）转换为噪声标准差
    snr = 10 ** (snr / 10)
...
def train_step(model, src, trg, n_var, pad, opt, criterion, channel, action):   #训练步骤
    model.train()
    trg_inp = trg[:, :-1]
    trg_real = trg[:, 1:]
    channels = Channels()
    ...
    enc_output = model.encoder(src, src_mask)                #语义编码器输出 enc_output

    channel_enc_output = model.channel_encoder(enc_output)   #信道编码器 channel_encoder编码

    channel_enc_output[:, :, action:] = 0

    Tx_sig = PowerNormalize(channel_enc_output)

    if channel == 'AWGN':           #经信道传输
    ...
    dec_output = model.decoder(trg_inp, channel_dec_output, look_ahead_mask, src_mask)    #语义解码

    pred = model.dense(dec_output)   #输入到一个全连接层,得到最终的预测值

    ntokens = pred.size(-1)

    loss = loss_function(pred.contiguous().view(-1, ntokens),    #计算损失
...
def val_step(model, src, trg, n_var, pad, criterion, channel):   #验证步骤
    channels = Channels()
    trg_inp = trg[:, :-1]
    trg_real = trg[:, 1:]
...
def greedy_decode(model, src, n_var, max_len, padding_idx, start_symbol, channel, action):          #贪心解码器
