...
def save_clean_sentences(sentence, save_path):   #将清洗后的句子保存到指定的文件路径
    pickle.dump(sentence, open(save_path, 'wb'))
    print('Saved: %s' % save_path)

def process(text_path):    #用于处理指定路径下的文本数据
...
def build_vocab(sequences, token_to_idx = { }, min_token_count=1, delim=' ',   #用于构建词汇表（vocabulary）并生成一个标记到索引的映射字典
                punct_to_keep=None, punct_to_remove=None, ):
    token_to_count = {}
