...
def train(args, model):                                                         #训练函数，使用给定的参数和模型进行训练
    epoch_list = []
    loss_list = []
    val_loss_list = []
    min_loss = 10 ** 8
...
def eval(args, model):                                                           #评估函数，使用给定的参数和模型对模型性能进行评估
    filename = os.path.basename(__file__).split('.')[0] + '_' + str(args.channel_type) + '_tcn' + str(
    ...
    for snrdb in range(args.snr_low_eval, args.snr_up_eval + 1):
        ...
        mse = np.mean(imse)               #计算每个snr对应的mse的平均值
...
#单循环DQN
def train_tcn(args, model, epoch):                                                          #训练函数，使用给定的参数和模型进行训练
    epoch_list = []
    loss_list = []
    val_loss_list = []
    min_loss = 10 ** 8
    filename = os.path.basename(__file__).split('.')[0] + '_' + str(args.channel_type) + '_tcn Adapt' + '_snrdb' + str(args.snr_low_train) + 'to' + str(
        args.snr_up_train) + '_bs' + str(args.batch_size) + '_lr' + str(args.learning_rate)
    model_path = args.model_dir + filename + '.h5'
    print(args.model_dir)
    print(model_path)
    ...
    snrdb_test = 0
    test_ds = None
    agent = DQN(alpha=0.0003, state_dim=env.state_dim, action_dim=env.action_dim, fc1_dim=256, fc2_dim=256, ckpt_dir=args.ckpt_dir)
    total_rewards, avg_rewards, eps_history = [], [], []  # 列表存储数据
    total_reward = 0
    done = False
    observation = env.reset(snrdb_test, test_ds)

    while not done:
        action = agent.choose_action(observation, isTrain=True)  # 根据观测值observation选择要执行的动作
        print('选择带宽： ', action)
        # 修改 tcn 的值######
        new_weights = np.random.rand(None, 8, 8, action)
        en5_conv = model.get_layer("en5_conv")
        en5_conv.set_weights([new_weights])
        # 修改 tcn 的值######


        print('###########开始传输数据##############')
        if args.channel_type == 'awgn':
            (train_ds, train_nums, snrdb_train), (
            test_ds, test_nums, snrdb_test) = dataset_cifar10.get_dataset_snr_range(args.snr_low_train,
                                                                                        args.snr_up_train)
        elif args.channel_type == 'slow_fading' or args.channel_type == 'slow_fading_eq':
            (train_ds, train_nums, snrdb_train), (
            test_ds, test_nums, snrdb_test) = dataset_cifar10.get_dataset_snr_range_and_h(
                args.snr_low_train, args.snr_up_train)
        ...
        if val_loss < min_loss:
            min_loss = val_loss
            model.save_weights(model_path)  #保存模型参数
            print('Epoch:', epoch + 1, ',loss=', loss, 'val_loss:', val_loss, 'save')
        else:
            print('Epoch:', epoch + 1, ',loss=', loss, 'val_loss:', val_loss)
        ...
        observation_ = env.reset(snrdb_test, test_ds)
...
def main(args):                                                          #构建编码器-解码器模型并训练或评估该模型
    ...
    tcn = args.transmit_channel_num
    encoder = Attention_Encoder(normal_imgs, input_snrdb, tcn)

    # 过信道
    print(args.channel_type)
    if args.channel_type == 'awgn':
    ...
    if args.command == 'train':
        ...
        eval(args, model)
    elif args.command == 'train_tcn':
        for epoch in range(args.epochs):
            print('开始第:{}次train'.format(epoch))
            train_tcn(args, model, epoch)

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("command", help='train/eval/train_tcn')
    parser.add_argument("-ct", '--channel_type', help="awgn/slow_fading/slow_fading_eq/burst", default='slow_fading')
    ...
    parser.add_argument("-de", "--dqn_episodes", help="epochs for dqn_training", default=50, type=int)
    ...
    parser.add_argument("-edb", "--eval_b_dir", help="eval_dir", default='eval/b/')
    ...
    parser.add_argument('--ckpt_dir', type=str, default='/home/src/ADJSCC-main/checkpoints/DQN/')
